...
tabulate::Table SetupTable(const std::string& zeroParagraph, const std::string& firstParagraph, const std::string& secondParagraph,
	FirstIter firstBegin, SecondIter secondBegin, const size_t limit) {
...
int main(const int argc, char* argv[]) {
	...
	for (auto i = 1; i < argc; ++i) {
		...
		const std::shared_ptr<IDataParser> parser =
			std::make_shared<JsonDataParser>(argv[i]);
		...
		const MathSolver mathSolver(data.begin(), data.end());
